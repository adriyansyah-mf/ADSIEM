...
try {
    $result = sc.exe create $ServiceName binPath= "`"$BinaryPath`" -config `"$ConfigPath`"" start= auto DisplayName= "SIEM Platform Agent"
    ...
    Write-Error "Failed to install SIEM Agent service: $_"
    exit 1
}
