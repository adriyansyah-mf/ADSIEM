...
async def ueba_ai_loop() -> None:
    """Disabled for now — cases are only created from alerts, not UEBA anomalies.

    UEBA anomalies still get scored and stored (see ueba_snapshot_loop /
    ueba_train_loop); this loop no longer auto-triages them into cases via
    analyze_and_maybe_create_case. See git history for the previous body.
    """
    return
