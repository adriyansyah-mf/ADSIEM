...
class ShodanProvider(ThreatIntelProvider):
    """Exposure/attack-surface lookup — open ports, banners, known CVEs on the host."""
    name = "shodan"

    def __init__(self, cfg: TIConfig) -> None:
        self._key = cfg.shodan_api_key or ""
        self._timeout = cfg.shodan_timeout_seconds
    ...
    async def lookup_ip(self, ip: str) -> dict[str, Any]:
        ...
        try:
            async with httpx.AsyncClient(timeout=self._timeout, verify=self._tls) as c:
                r = await c.get(
                    f"https://api.shodan.io/shodan/host/{parsed.compressed}",
                    params={"key": self._key, "minify": "true"},
                )
